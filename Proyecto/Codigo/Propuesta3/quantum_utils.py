...
class QuantumDownsampler:
    """
    Clase para manejar la capa de downsampling cuántico (Propuesta 3).
    ...
    def process_image(self, image, n_layers=2, verbose=False):
        """
        Procesa una imagen completa aplicando múltiples capas de convolución cuántica.
        Ejemplo: 128x128 --(capa 1)--> 64x64 --(capa 2)--> 32x32
        """
        ...
        if verbose:
            print(f"\n[Quantum] Entrada original: {image.shape}")

        for i in range(n_layers):
            # 1. Preprocesar (Sliding Window / Bloques)
            blocks, out_h, out_w = self._preprocess_image_blocks(current_image)
            ...
            # 4. Reconstrucción temporal para la siguiente capa
            current_image = _np.array(results).reshape(out_h, out_w)
            
            if verbose:
                print(f"[Quantum] Capa {i+1} completada. Nueva dimensión: {current_image.shape}")
